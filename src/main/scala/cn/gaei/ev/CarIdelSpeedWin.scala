...
//      .config("spark.serializer", "org.apache.spark.serializer.KryoSerializer")
    ...
    spark.udf.register("state", (e: Seq[Int]) => {
      var res = 1
      if (e.size == 2) {
        if((e(0) == 0) &&  (e(1) == 0)){
          res = 0
...
//    val winSpec3 = Window.partitionBy($"vin",$"seq_id").orderBy($"ts").rowsBetween(Window.currentRow, Window.unboundedFollowing)
    ...
    val orig = spark.read.parquet(inputorig).filter($"bcm_keyst".equalTo(2)).select($"vin", $"ts",$"bcm_keyst",$"bcs_vehspd",$"ems_engspd")
      ...
      .withColumn("fre_state", callUDF("state",collect_list($"state").over(winSpec)))
      .cache()
    //all
    val mix = orig
...
//    mix_run_idle.withColumn("fre_state", callUDF("state",collect_list($"flags").over(winSpec)))
    val mix_idle = mix.filter($"fre_state".equalTo(0)).count()
    val mix_idle_pure = mix.filter($"fre_state".equalTo(0) && $"ems_engspd".equalTo(0) ).count
    val mix_idle_hybrid = mix.filter($"fre_state".equalTo(0) && $"ems_engspd".notEqual(0) ).count
...
//    pri_run_idle.withColumn("fre_state", callUDF("state",collect_list($"flags").over(winSpec)))
    val pri_idle = run_private.filter($"fre_state".equalTo(0)).count()
    val pri_idle_pure = run_private.filter($"fre_state".equalTo(0) && $"ems_engspd".equalTo(0) ).count
    val pri_idle_hybrid = run_private.filter($"fre_state".equalTo(0) && $"ems_engspd".notEqual(0) ).count
...
//    tax_run_idle.withColumn("fre_state", callUDF("state",collect_list($"flags").over(winSpec)))
    val tax_idle = run_taxi.filter($"fre_state".equalTo(0)).count()
    val tax_idle_pure = run_taxi.filter($"fre_state".equalTo(0) && $"ems_engspd".equalTo(0) ).count
    val tax_idle_hybrid = run_taxi.filter($"fre_state".equalTo(0) && $"ems_engspd".notEqual(0) ).count
